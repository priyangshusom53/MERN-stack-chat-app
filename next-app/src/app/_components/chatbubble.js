...
export default function ChatBubble({ message, type }) {
   return (
      <>
         <div className={cn(`max-w-[70%] md:max-w-[60%] ${bg_fg_color()} px-2 md:px-4 py-2 rounded-lg`, `${(type === 'sent') ? `rounded-br-xs ml-auto ${bg_fg_color('bg-foreground text-background')}` : `rounded-bl-xs dark:dark ${interaction_color()}`} border`)}>
